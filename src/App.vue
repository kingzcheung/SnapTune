...
const onClick = () => {
  ...
  files.value.forEach((file, index) => {
    invoke("image2x_command", { x: currentFormat.value, index: index, source: file.file })
      .then((index) => {
...
<template>
  <div class="relative h-full overscroll-none	">
    <div class="tabs tab-lifted">
      <a class="tab" @click="clickTab(i)" :class="{ 'tab-active': i == currentTab }" v-for="(t, i) of tabData" :key="t">
        {{ t }}
      </a>
    </div>
    <div class="overflow-y-auto">
      <table class="table table-compact w-full mb-20">
        ...
        <tbody>
          <!-- row 1 -->
          <tr :class="{ 'active': i % 2 == 0 }" v-for="(file, i) of files">
            <th>{{ i + 1 }}</th>
            <td class="truncate">{{ file.file }}</td>
            <td>{{file.type}}</td>
            <td>
              <svg v-if="file.status === 1" t="1656489464034" class="icon" viewBox="0 0 1024 1024" version="1.1"
                xmlns="http://www.w3.org/2000/svg" p-id="1248" width="16" height="16">
                <path d="M512 512m-448 0a448 448 0 1 0 896 0 448 448 0 1 0-896 0Z" fill="#07C160" p-id="1249"></path>
                <path
                  ...
                  fill="#FFFFFF" p-id="1250"></path>
              </svg>
              <svg v-if="file.status === -1" t="1656489517929" class="icon" viewBox="0 0 1024 1024" version="1.1"
                xmlns="http://www.w3.org/2000/svg" p-id="1523" width="16" height="16">
                <path d="M512 512m-448 0a448 448 0 1 0 896 0 448 448 0 1 0-896 0Z" fill="#FA5151" p-id="1524"></path>
                <path
                  ...
                  fill="#FFFFFF" p-id="1525"></path>
              </svg>
              <svg v-if="file.status === 0" t="1656489552631" class="icon" viewBox="0 0 1024 1024" version="1.1"
                xmlns="http://www.w3.org/2000/svg" p-id="1798" width="16" height="16">
                <path d="M512 512m-448 0a448 448 0 1 0 896 0 448 448 0 1 0-896 0Z" fill="#CCCCCC" p-id="1799"></path>
                <path
    ...
    <div class="fixed z-50 left-0 right-0 bottom-0 bg-gray-800 w-full flex items-stretch">
      <div class="flex-1 flex items-center">
        ...
        <div v-if="errors.error" class="pl-4 flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current flex-shrink-0 h-6 w-6" fill="#ff0000" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
          <span class="text-gray-300 pl-1">错误: {{errors.msg}}</span>
        </div>
      </div>
      <div class="flex items-center dropdown dropdown-left dropdown-end">
        <label tabindex="0" class="btn m-1">{{ currentFormat ? currentFormat : '选择格式' }}</label>
        <div tabindex="0" class="dropdown-content card card-compact w-96 p-2 shadow bg-black text-primary-content">
          <div class="card-body">
            <div class="grid grid-cols-3 gap-4">
              <button @click="selectFormat(format)" v-for="format in formats" :key="format" class="btn text-lg"
                :class="{ 'btn-active btn-success': format === currentFormat }">{{ format }}</button>
            </div>
          </div>
        </div>
      </div>

      <div class="flex items-center px-1 bg-blue-700 ml-2	cursor-pointer" @click="onClick">
        <span class="text-primary-content font-bold px-8 ">转换</span>
        <svg t="1656476104531" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
          p-id="2198" width="32" height="18">
